create table BILL_INFORMATION (ID integer not null auto_increment, AMOUNT float(7,2) not null, BILLING_DATE datetime not null, PENDING_AMOUNT float(7,2), ORDER_INFO_ID integer not null, primary key (ID)) engine=InnoDB;
...
create table ORDER_INFO (ID integer not null auto_increment, DESCRIPTION varchar(255), MODIFIED_AT datetime, ORDER_DATE datetime not null, STATUS enum ('ACTIVE', 'INACTIVE') not null, MODIFIED_BY integer, ORDERED_BY integer not null, primary key (ID)) engine=InnoDB;
